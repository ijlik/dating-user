...
import (
	...
	"github.com/ijlik/dating-user/pkg/config"
...
func GetLogger() Logger {
	if globalLogger == nil {
		logger := newLogger(config.NewConfig("", 5))
		globalLogger = &logger
	}
	return *globalLogger
}
...
func newLogger(env configdata.Config) Logger {

	configs := zap.NewDevelopmentConfig()
	logOutput := os.Getenv("LOG_OUTPUT")

	if env.GetString("ENVIRONMENT") == "development" {
		fmt.Println("encode level")
		configs.EncoderConfig.EncodeLevel = zapcore.CapitalColorLevelEncoder
	}

	if env.GetString("ENVIRONMENT") == "production" && logOutput != "" {
		configs.OutputPaths = []string{logOutput}
	}
	...
	configs.Level.SetLevel(level)

	zapLogger, _ = configs.Build()
